...
class RenderPanel:
    ...
    def populate_render_tab(self,
        ...
        with server.add_gui_folder("Preview Options"):
            ...
            load_button = server.add_gui_button(
            ...
            def load_json_data(json_data):
                for client in server.get_clients().values():
                    ...
                    with client.atomic():
                        client.camera.wxyz = tf.SO3.from_matrix(onp.array(target_camera['rotation'])).wxyz
                        client.camera.position = onp.array(target_camera['position'])
                ...
                model_sizes = [i.value for i in model_size_sliders]
                model_poses_copied = [i.copy() for i in model_poses]
                camera_path.add_camera(Keyframe(
                        # camera.position,
                        # camera.wxyz,
                        override_fov_enabled=False,
                        # override_fov_value=camera.fov,
                        aspect=resolution.value[0] / resolution.value[1],
                        enable_model_transform=True,
                        model_sizes=model_sizes,
                        model_poses=model_poses_copied,
                    ))
