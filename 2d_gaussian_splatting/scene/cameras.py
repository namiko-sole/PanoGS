...
class Simple_Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, h, w,
                 image_name, uid,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device = "cuda:1", qvec=None
                 ):
        ...
        self.cx = w//2
        self.cy = h//2
        self.appearance_id=torch.tensor([0], dtype=torch.int)
        self.normalized_appearance_id=torch.tensor([0.], dtype=torch.float)
        self.time = torch.zeros(self.R.shape[0])
        self.distortion_params = torch.zeros(self.R.shape[0], 4)
        self.camera_type=torch.tensor([0], dtype=torch.int)
        self.fx = torch.tensor([fov2focal(FoVx, w//2)], dtype=torch.float)
        self.fy = torch.tensor([fov2focal(FoVy, h//2)], dtype=torch.float)
        ######
