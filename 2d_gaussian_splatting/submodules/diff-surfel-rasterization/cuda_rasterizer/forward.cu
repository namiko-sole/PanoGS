	...
	float* __restrict__ out_others)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// float depth = (rho3d <= rho2d) ? (s.x * Tw.x + s.y * Tw.y) + sqrtf(s.x*Tw.x*s.x*Tw.x+s.y*Tw.y*s.y*Tw.y+Tw.z*Tw.z) : sqrtf(s.x*Tw.x*s.x*Tw.x+s.y*Tw.y*s.y*Tw.y+Tw.z*Tw.z); 
			// float depth = (rho3d <= rho2d) ? (s.x * Tw.x + s.y * Tw.y) : 0;
