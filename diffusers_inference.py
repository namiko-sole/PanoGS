...
def generate_image(prompt, style_img_path, input_img_path, ref_img_path, depth_img_path, mask_img_path=None, mask_reverse=True, **kwargs):
    global depth_pipe
    ...
    with torch.no_grad():
        if mask_img_path is None:
            ...
            # Decode latent once for saving while reusing latent directly for refine.
            decode_latents = inpaint_latents
            if inpaint_pipe.vae.dtype == torch.float16 and inpaint_pipe.vae.config.force_upcast:
                inpaint_pipe.upcast_vae()
                decode_latents = decode_latents.to(next(iter(inpaint_pipe.vae.post_quant_conv.parameters())).dtype)
            decode_image = inpaint_pipe.vae.decode(
                decode_latents / inpaint_pipe.vae.config.scaling_factor,
                return_dict=False,
            )[0]
            image = inpaint_pipe.image_processor.postprocess(decode_image, output_type="pil")[0]
