...
class StableDiffusionXLControlNetImg2ImgLoopConsistentPipeline(
    DiffusionPipeline,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i!=0 and i<=len(timesteps)-2:
                    ...
                    rotated_latents[:,:,:,:w*3//4] = latents[:,:,:,w//4:]
                    rotated_latents[:,:,:,w*3//4:] = latents[:,:,:,:w//4]
                    latents = rotated_latents

                    for i in range(len(control_images)):
                        cond_img = control_images[i]
                        rotated_image = torch.zeros_like(cond_img)
                        b,c,h,w = cond_img.shape
                        rotated_image[:,:,:,:w*3//4] = cond_img[:,:,:,w//4:]
                        rotated_image[:,:,:,w*3//4:] = cond_img[:,:,:,:w//4]
                        control_images[i] = rotated_image
